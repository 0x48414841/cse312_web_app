...
type Lobby struct {
	GameInstance   *Game
	ConnectedUsers []*net.Conn
}

func MakeLobby(c *net.Conn, lobbyId string) *Lobby {
	l := &Lobby{GameInstance: NewGame(), ConnectedUsers: make([]*net.Conn, 0)}
	//TODO make concurrent access safe
	Lobbies[lobbyId] = l
	return l
...
func (l *Lobby) AddPlayer(c *net.Conn) error {
	if len(l.ConnectedUsers) < MAX_LOBBY_SIZE {
